...
class FQLAgent(nn.Module):
    ...
    def update(self, batch):
        ...
        loss.backward()
        nn.utils.clip_grad_norm_(self.parameters(), max_norm=10.0)  #Q values exploding it seems
        self.optimizer.step()
    ...
    def sample_actions(self, observations, seed=None, temperature=1.0):
        ...
        with torch.no_grad():
            ...
            if seed is not None:
                torch.manual_seed(seed)
            noises = torch.randn(batch_size, self.action_dim, device=self.device) * temperature
    ...
    def state_dict(self):
        """Override state_dict to include only network parameters."""
        return {
    ...
    def load_state_dict(self, state_dict):
        """Override load_state_dict to handle network parameters."""
        self.critic.load_state_dict(state_dict['critic'])
        self.target_critic.load_state_dict(state_dict['target_critic'])
        self.actor_bc_flow.load_state_dict(state_dict['actor_bc_flow'])
        self.actor_onestep_flow.load_state_dict(state_dict['actor_onestep_flow'])